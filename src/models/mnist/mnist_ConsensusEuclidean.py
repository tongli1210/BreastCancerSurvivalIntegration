...
class Model(NetInterface):
    ...
    def __init__(self, opt, logger):
        ...
        # general
        self.opt =opt  
        self._metrics = ['loss_euclidean', 'loss_clf'] # log the autoencoder loss and the classification loss
        if opt.log_time:
            self._metrics += ['t_euclidean', 't_clf']
    ...
    def _train_on_batch(self, epoch, batch_idx, batch):
        ...
        ########################################################
        # Stage 1: train Q to minimize the Euclidean distance  #
        ########################################################
        for i in range(self.n_modality):
            net_q[i].zero_grad()
