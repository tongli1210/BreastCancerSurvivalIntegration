...
class Model(NetInterface):
    ...
    def __init__(self, opt, logger):
        ...
        # general
        self.opt =opt  
        self._metrics = ['loss_mse', 'loss_euclidean', 'loss_clf'] # log the autoencoder loss and the classification loss
        if opt.log_time:
            self._metrics += ['t_mse', 't_euclidean', 't_clf']
    ...
    def _train_on_batch(self, epoch, batch_idx, batch):
        ...
        ########################################################
        # Stage 2: train Q to minimize the Euclidean distance  #
        ########################################################
        for i in range(self.n_modality):
            net_q[i].zero_grad()
            for p in net_q[i].parameters():
        ...
        for i in range(self.n_modality):
            z_list[i] = net_q[i](X_list[i])
        # Minimize the Euclidean distance between z_mean and any elements in z_list
        loss_euclidean = euclidean_distance_to_mean(z_list)
        loss_euclidean.backward()
        for i in range(self.n_modality):
            opt_q[i].step()
        opt_c.step()
        t_euclidean = time() - t0
        batch_log['loss_euclidean'] = loss_euclidean.item()
        ...
        if self.opt.log_time:
            ...
            batch_log['t_euclidean'] = t_euclidean
...
class Model_test(Model):
    ...
    def __str__(self):
        return "Testing AutoencoderConsensusEuclidean"
